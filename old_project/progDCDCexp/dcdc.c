...
void conversion_custom_to_value_DCDC(Ixxat_CAN_Msg_t *pCustomMsg, uint16_t *pValueMsg) //  Out_custom -> value
{
  ...
  if (pCustomMsg->ID & maskPGN == custum_DCDC.rx_customMsg323.ID & maskPGN)
  {
  ...
  if (pCustomMsg->ID & maskPGN == custum_DCDC.rx_customMsg313.ID & maskPGN)
  {
...
{
  if (custum_DCDC.rx_customMsg323.ID & maskPGN == RxCan.ID & maskPGN)
  ...
  if (custum_DCDC.rx_customMsg313.ID & maskPGN == RxCan.ID & maskPGN)
